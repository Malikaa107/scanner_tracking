...
MATERIAL_RESEP_DUMMY = [
    {"nama": "TEPUNG TERIGU", "barcode": "20250418-293111"},
    {"nama": "MINYAK NABATI", "barcode": "20250418-235100"},
    {"nama": "BUBUK COKLAT", "barcode": "20250418-125894"},
    {"nama": "GULA PASIR", "barcode": "20250418-198214"},
    {"nama": "SUSU BUBUK", "barcode": "20250418-277660"},
    {"nama": "LECITHIN", "barcode": "20250418-329259"}, 
    {"nama": "AIR", "barcode": "-"}, 
]
...
class JoblistSelector(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(header_frame, text="Joblist Formulasi", font=("Franklin Gothic Heavy", 28, "bold"), text_color="white").pack(side="left")
    ...
    def fetch_jobs_from_db(self):
        try:
            conn = get_connection()
            cursor = conn.cursor()
            query = 'SELECT j.nomor_job, j.tanggal, j."resepId", j.target_qty, j.status, r.nama_resep FROM qc.formulasi_joblist j LEFT JOIN qc.master_resep r ON j."resepId" = r.id WHERE j.status IN (0, 1, 2) ORDER BY j.status ASC, j.tanggal DESC'
...
class AppScanner(ctk.CTk):
    ...
    def fetch_materials_by_resep(self, resep_id):
        ...
        except: return MATERIAL_RESEP_DUMMY

    def show_joblist_selector(self):
        self.is_scanning = self.batch_active = False 
        self.current_batch_num = 1
    ...
    def prepare_scanning_area(self, job_no, target_qty, resep_id):
        ...
        self.selected_job_no, self.target_qty_total = job_no, int(target_qty)
    ...
    def setup_scanner_ui(self):
        ...
        ctk.CTkLabel(self.header, text="SISTEM SCANNER FORMULASI", font=("Franklin Gothic Heavy", 28, "bold"), text_color="white").place(x=title_x, y=30)
    ...
    def auto_scan_handler(self, event, force=False):
        if force:
            if self._scanner_timer: self.after_cancel(self._scanner_timer)
            self.process_scan()
            return
        if self._scanner_timer: self.after_cancel(self._scanner_timer)
        self._scanner_timer = self.after(150, self.process_scan)
    ...
    def process_scan(self):
        ...
        if target: 
            if code in self.scanned_materials: self.show_notification("SUDAH DI SCAN!", "#f59e0b")
            else:
                self.scanned_materials.add(code)
                self.show_notification(f"{target['nama']}", "#22c55e")
                self.update_sidebar_lists() 
                self.check_all_materials_completed()
        else: self.show_notification("BARCODE TIDAK VALID!", "#ef4444")
        self.entry_barcode.focus_set()
    ...
    def update_sidebar_lists(self):
        ...
        if self.completed_batches:
            ...
            for b in self.completed_batches: ctk.CTkLabel(self.scroll_sidebar_left, text=f"✓ {b} SELESAI", text_color="#22c55e", font=("Arial", 11)).pack(anchor="w", padx=10)
        ...
        for item in self.material_resep:
            is_manual, is_done = item["barcode"] in ["-", "", None], item["barcode"] in self.scanned_materials
            if not is_manual:
                ...
                f = ctk.CTkFrame(self.scroll_sidebar_right, fg_color="#334155" if not is_done else "#064e3b", corner_radius=6, height=40)
                f.pack(fill="x", pady=2, padx=5)
                lbl = ctk.CTkLabel(f, text=f"{'✓ ' if is_done else ''}{item['nama']}", text_color="white" if not is_done else "#34d399", font=("Arial", 12))
                lbl.pack(side="left", padx=5)
                btn_check = ctk.CTkCheckBox(f, text="", width=24, height=24, checkbox_width=24, checkbox_height=24, border_width=2, corner_radius=4, border_color="#94a3b8", fg_color="#22c55e", command=lambda b=item['barcode']: self.manual_check_handler(b)) 
                if is_done: btn_check.select()
    ...
    def konfirmasi_stop(self):
        ...
        x, y = (screen_width // 2) - 200, (screen_height // 2) - 110
        pop.geometry(f"+{x}+{y}") 
        ...
        ctk.CTkButton(btn_frame, text="KELUAR", width=130, height=40, fg_color="#ef4444", hover_color="#dc2626", font=("Arial", 12, "bold"), command=lambda: [pop.destroy(), self.show_joblist_selector()]).pack(side="left", padx=10)
